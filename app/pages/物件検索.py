...
SP_SHEET = '緯度・経度取得' # sheet名
USER_FAVORITES_SHEET = 'ユーザーお気に入り'  # ユーザーお気に入りの保存先シート
...
# ユーザーのお気に入りを保存する関数
def save_to_favorites(user_id, property_info):
    SP_CREDENTIAL_FILE = PRIVATE_KEY_PATH
    scopes = [
        'https://www.googleapis.com/auth/spreadsheets',
        'https://www.googleapis.com/auth/drive'
    ...
    gc = gspread.authorize(credentials)
    SP_SHEET_KEY = SPREADSHEET_User_ID
    sh = gc.open_by_key(SP_SHEET_KEY)
    worksheet = sh.worksheet(USER_FAVORITES_SHEET)
    
    # ユーザーIDと物件情報を追加
    worksheet.append_row([user_id] + property_info)
...
# 地図を作成し、マーカーを追加する関数
def create_map(filtered_df, user_id):
    # 地図の初期設定
    ...
    for idx, row in filtered_df.iterrows():
        if pd.notnull(row['latitude']) and pd.notnull(row['longitude']):
            ...
            <b>家賃:</b> {row['家賃']}万円<br>
            <b>間取り:</b> {row['間取り']}<br>
            <a href="{row['物件詳細URL']}" target="_blank">物件詳細</a><br>
            <button onclick="streamlit.send({row['名称']}, {row['アドレス']}, {row['家賃']}, {row['間取り']}, {row['物件詳細URL']})">お気に入り</button>
            """
            # HTMLをポップアップに設定
...
def main():
    ...
    if st.session_state.get('search_clicked', False):
        user_id = "example_user_id"  # 実際のユーザーIDの取得方法に置き換えてください
        m = create_map(st.session_state.get('filtered_df2', filtered_df2), user_id)
        folium_static(m)
